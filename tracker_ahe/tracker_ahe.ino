...
void loop() {
  ...
  while (time_share < timeout) {
    while (gps.available() > 0) {
      ...
      if (digitalRead(sim_packet) == 0) {
        //simulation mode
        if (strcmp(gps_cmd, "$GNGGA") == 0) {
          strcpy(raw, "$GNGGA,042303.000,3938.7688,N,07958.4412,W,1,09,1.33,328.5,M,-33.0,M,,*4F");
        }
        //Serial.println(raw);
      }
...
int update_GPS(String gps_data) {
